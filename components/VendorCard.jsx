import Link from "next/link";
import React from "react";
import styles from "./VendorCard.module.css";

function VendorCard({ vendor, type, contact, toggle, target }) {
  return (
    <div className={styles.VendorCard}>
      {type == "STARTUP" && (
        <>
          <div className={styles.imgContainer}>
            {vendor.profile != null ? (
              <img src={vendor.profile} />
            ) : (
              <img src="data:image/png;base64," />
            )}
          </div>
          <div className={styles.textContainer}>
            <h1>{vendor.company_name}</h1>
            <span>
              {vendor.city_name}, {vendor.state_name}, {vendor.country_name}
            </span>
            <h2>
              {vendor.services.map((service) => {
                return service.service_subcat_name + " and more...";
              })}
            </h2>
            <p>{vendor.description}</p>
          </div>
          <div className={styles.btngroup}>
            <button>
              <Link href={`vendor/startup/${vendor.startup_id}`}>
                view profile
              </Link>
            </button>
            <button
              type="button"
              data-bs-toggle={toggle}
              data-bs-target={target}
              onClick={() => contact(vendor.city_id, vendor.user_id)}
            >
              interested
            </button>
          </div>
        </>
      )}

      {type == "FREELANCER" && (
        <>
          <div className={styles.imgContainer}>
            {vendor.profile != null ? (
              <img src={vendor.profile} />
            ) : (
              <img src="data:image/png;base64," />
            )}
          </div>
          <div className={styles.textContainer}>
            <h1>
              {vendor.first_name} {vendor.last_name}
            </h1>
            <span>
              {vendor.city_name}, {vendor.state_name}, {vendor.country_name}
            </span>
            <h2>
              {vendor.services.map((service) => {
                return service.service_subcat_name + " and more...";
              })}
            </h2>
            <p>{vendor.about}</p>
          </div>
          <div className={styles.btngroup}>
            <button>
              <Link href={`vendor/freelancer/${vendor.user_id}`}>
                view profile
              </Link>
            </button>
            <button
              type="button"
              data-bs-toggle="modal"
              data-bs-target="#exampleModal"
              onClick={contact}
            >
              interested
            </button>
          </div>
        </>
      )}
    </div>
  );
}

export default VendorCard;
